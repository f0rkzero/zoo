...
local t = tick()
while gui:FindFirstChild("DetailTextLabel", true) and tick() - t < 30 do task.wait() end
...
local contain123
local Hub = "Moondiety"
local Hub_Script_ID = "04fac618b04f3d64a37bb729a758853f"
local Discord_Invite = "https://discord.gg/moondiety"
local UI_Theme = "Dark"

local Linkvertise_Enabled = true
local Linkvertise_Link = "https://ads.luarmor.net/get_key?for=Key-IHeAAOKIUTol"

local Lootlabs_Enabled = true
local Lootlabs_Link = "https://ads.luarmor.net/get_key?for=Key-LWGKRDAaXqFl"
-------------------------------------------
...
local function checkKey(input_key, button)
    ...
    if status.code == "KEY_VALID" then
        ...
        if button then
            ...
            if label then
                ...
                label.Text = "Verifying Key..."
                task.delay(3, function()
                    label.Text = originalText
                end)
                 if Library and contain123 then
            Library.close(contain123) end
            end
        end
        ...
        if button then
            ...
            if label then
                local originalText = label.Text
                local message = messages[status.code] or "Unknown error"
                label.Text = message
...
function Library:open()
    ...
    if UserInputService.TouchEnabled then
        containerSize = UDim2.new(0.8, 0, 0.85, 0)
    else
...
-- ðŸŒ™ Moondiety Toggle Icon
local player = game.Players.LocalPlayer
local TweenService = game:GetService("TweenService")

-- Create floating toggle icon GUI
local toggleGui = Instance.new("ScreenGui")
toggleGui.Name = "MoondietyIconGUI"
...
task.spawn(function()
	while true do
		local grow = TweenService:Create(icon, TweenInfo.new(0.4, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {
			ImageTransparency = 0.3,
			Size = UDim2.new(0, 42, 0, 42),
			Position = UDim2.new(0, 4, 0, 4)
		})
		local shrink = TweenService:Create(icon, TweenInfo.new(0.4, Enum.EasingStyle.Sine, Enum.EasingDirection.In), {
...
toggleFrame.InputBegan:Connect(function(input)
    ...
    if UserInputService.TouchEnabled then
        Container.Size = UDim2.new(0.8, 0, 0.85, 0)
    else
...
function Module:create_dropdown()
    ...
    Dropdown.selectedOption = nil -- Ñ‚ÐµÐºÑƒÑ‰Ð¸Ð¹ Ð²Ñ‹Ð±Ð¾Ñ€
    ...
    function Dropdown:update()
        ...
        for _, value in ipairs(self.options) do
            ...
            -- ÐŸÐ¾Ð´ÑÐ²ÐµÑ‡Ð¸Ð²Ð°ÐµÐ¼ Ñ‚Ð¾Ð»ÑŒÐºÐ¾ Ñ‚ÐµÐºÑƒÑ‰Ð¸Ð¹ Ð²Ñ‹Ð±Ñ€Ð°Ð½Ð½Ñ‹Ð¹ option
    ...
    -- Ð˜Ð½Ð¸Ñ†Ð¸Ð°Ð»Ð¸Ð·Ð¸Ñ€ÑƒÐµÐ¼ Ñ Ð¿ÑƒÑÑ‚Ñ‹Ð¼ Ð²Ñ‹Ð±Ð¾Ñ€Ð¾Ð¼
    ...
    -- ðŸ”‘ Info label encouraging user to do the keysystem
    local InfoLabel = Instance.new("TextLabel")
    InfoLabel.Name = "KeyInfo"
    InfoLabel.Parent = section
    InfoLabel.BackgroundTransparency = 1
    InfoLabel.Position = UDim2.new(0.02, 0, 0, 0)
    InfoLabel.Size = UDim2.new(0.96, 0, 0, 25)
    InfoLabel.FontFace = Font.new("rbxasset://fonts/families/Montserrat.json", Enum.FontWeight.Bold)
    InfoLabel.Text = "ðŸ”‘ One step key checkpoint â€” takes 15 sec to complete!"
    InfoLabel.TextColor3 = Color3.fromRGB(191, 1, 0)
    ...
    -- Save text to flags on any change
   

    -- Ð ÐµÐ°Ð³Ð¸Ñ€ÑƒÐµÐ¼ Ð½Ð° Ð»ÑŽÐ±Ð¾Ðµ Ð¸Ð·Ð¼ÐµÐ½ÐµÐ½Ð¸Ðµ Ñ‚ÐµÐºÑÑ‚Ð°
...
checkKeyButton = tab.create_button({ 
    name = 'Check Key',
    flag = 'tptobase',
    section = 'left',
    enabled = false,
    ...
    callback = function(state: boolean)
         local key = Library.Flags["key_input"]
                  
                    if key ~= nil and key~="" then
                 
                    checkKey(key, checkKeyButton) end
    end
})
local linkvertiseButton 

linkvertiseButton = tab.create_button({
    ...
    callback = function(state: boolean)
        ...
        local label = linkvertiseButton:FindFirstChildWhichIsA("TextLabel")
...
lootButton = tab.create_button({
    ...
    callback = function(state: boolean)
        ...
        local label = lootButton:FindFirstChildWhichIsA("TextLabel")
        if not label then return end
...
discordButton = tab.create_button({
    ...
    callback = function(state: boolean)
        ...
        local label = discordButton:FindFirstChildWhichIsA("TextLabel")
        if not label then return end

        local originalText = label.Text
        label.Text = "Invite copied!"
...
return library
function Library:toggled()
	if Library.core then
		Library.core.Enabled = not Library.core.Enabled
	end
end
